...
namespace XperienceAdapter.Repositories
{
        ...
        where TPage : TreeNode, new()
    {
        ...
        {
            var siteId = 1;
            var siteName = "MedioClinic" ;
            var result = _repositoryServices.PageRetriever.Retrieve(
                query =>
            {
                query.OnSite(siteId).Columns(DefaultDtoFactory().SourceColumns);
                filter?.Invoke(query);
            },
            buildCacheAction
            );
        ...
        protected virtual TPageDto MapBasicDtoProperties(TPage page, bool includeAttachments)
        {
            ...
            if (includeAttachments)
            {
                ...
                {
                    dto.Attachments.Add(new PageAtthachment
                    {
